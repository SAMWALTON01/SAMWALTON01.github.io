...
> ⚠️ **פעולה נדרשת מהמתכנת של הלקוח (דחוף!):** להסיר את ה-Custom Domain
> `db.nahmanbot.com` מהפרויקט הישן ב-Supabase Cloud (Settings → Custom Domains).
> כל עוד הוא מוגדר שם, חלק מצומדי ה-edge של Supabase מנתבים אליו בקשות —
> **וזה כבר קרה בפועל (20.7):** קמפיינים camp12/camp07 יצאו דרך העותק הישן
> של הפונקציה בענן (לפני עדכון הערב של 19.7) ולכן נשלחו עם לינק github.io
> ונוסח ברירת מחדל ישן. יש לפרוס מחדש את הקוד העדכני גם לענן (§6) או להסיר
> את ה-domain — ואז כל הבקשות יגיעו לשרת בלבד.
