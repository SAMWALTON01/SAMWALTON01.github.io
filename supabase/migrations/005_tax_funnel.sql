-- 005_tax_funnel.sql — תסריט החזר מס בפאנל (19.7)
-- 1. has_insurance נוסף למיפוי השדות ב-submit_web_lead
-- 2. calc_summary_web: "לא ביצעתי החזר מס" כבר לא מסווג בטעות כ"לא מעוניין"
-- אדיטיבי, create or replace בלבד. להרצה בטרנזקציה אחת (psql -1).
begin;
...
begin
  ...
  if position('אל תבדקו' in chack) > 0 or position('לא רוצה' in chack) > 0 or did like 'לא רוצה%' then
